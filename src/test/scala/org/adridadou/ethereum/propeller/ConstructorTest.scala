...
class ConstructorTest extends FlatSpec with Matchers with Checkers {
  ...
  "Constructor" should "use the default constructor if no arguments are passed" in {
    val ethereum = CoreEthereumFacadeProvider.create(new EthereumJs(EthereumJsConfig.builder.balance(mainAccount, ether(1000)).build), EthereumConfig.builder().build())
